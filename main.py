...
u_final_val_ndcg = 0.0
u_final_val_hr = 0.0
u_final_test_ndcg = 0.0
u_final_test_hr = 0.0
u_best_epoch = 0
...
with tf.Session(config=config) as sess:
    ...
    if is_restore:
        ...
        u_best_epoch2 = 0
        u_final_test_ndcg2 = t_test[2]
        u_final_test_hr2 = t_test[3]
        best_epoch = 0
        final_val_ndcg = t_valid[0]
    ...
    try:
        for epoch in range(1, args.num_epochs + 1):
            ...
            if epoch % args.eval_interval == 0 or global_step >= num_batch * args.num_epochs:
                ...
                if t_test[2] > u_final_test_ndcg2:
                    u_best_epoch2 = epoch
                    u_final_test_ndcg2 = t_test[2]
                    u_final_test_hr2 = t_test[3]
                                
                if t_valid[0] > final_val_ndcg:
                    best_epoch = epoch
                    final_val_ndcg = t_valid[0]
                    final_val_hr = t_valid[1]
                    final_test_ndcg = t_test[0]
