...
do
    ...
    do
        ...
        python main.py --dataset=${dataset} --train_dir=default --hidden_units=50 --u_hidden_units=6 --maxlen=${maxlen} --dropout_rate=${dropout} --model=${model} --num_epochs=${epoch} --c0=30 --disentangle=True --debias=True --additive_bias=False --pop_loss_w=0.02 --dynamic_pop_int_weight=True --pop_match_loss_w=2.0 --int_match_loss_w=2.0 --pop_match_tower=True > logs_ipw_t/${dataset}_${model}_dist_ipw_2.0_$i.log 2>&1 &
...
find logs_ipw_t -type f -name "*log" -exec awk '{s=$0};END{print FILENAME,s}' {} \; | sort